...
import (
	...
	"anomaly-go/internal/models"

	"github.com/gin-gonic/gin"
)

// API holds dependencies for handlers, like the database connection.
type API struct {
	DB  *database.DBStore
...
func (a *API) LoginHandler(c *gin.Context) {
	...
	// IMPORTANT: Replace this with real user validation from your database!
	if req.Username != "admin" || req.Password != "password" {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "Invalid credentials"})
		return
	}

	token, err := auth.GenerateToken(req.Username, a.Cfg.JWTSecret)
	if err != nil {
...
// UpdateConfidenceThresholdHandler updates the confidence threshold value.
func (a *API) UpdateConfidenceThresholdHandler(c *gin.Context) {
	...
	res, err := a.DB.Exec(updateQuery, req.ConfidenceThreshold)
	if err != nil {
	...
	if rowsAffected == 0 {
		c.JSON(http.StatusNotFound, gin.H{"error": "No threshold value found to update. The table might be empty."})
		return
	}
...
func (a *API) FetchDataHandler(c *gin.Context) {
	...
	rows, err := a.DB.Query(baseQuery, params...)
	if err != nil {
	...
	err = a.DB.QueryRow(countQuery, params...).Scan(&totalReviewRequired, &totalAnomalyDetected, &totalFraud, &totalNullAnomalyCheck)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Count query error: " + err.Error()})
		return
	}

	response := models.FetchDataResponse{
...
// GetAllDeviceIdsHandler fetches distinct device IDs from anomaly results.
...
// GetDeviceHealthIdsHandler fetches distinct device IDs from battery health data.
...
// UpdateReviewHandler updates the review status of a transaction.
func (a *API) UpdateReviewHandler(c *gin.Context) {
	...
	res, err := a.DB.Exec(updateQuery, newReview, req.TransactionID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Database update error: " + err.Error()})
		return
	}
...
func (a *API) GetAtRiskKPIsHandler(c *gin.Context) {
	...
	err := a.DB.QueryRow(totalQuery).Scan(&totalDevices)
	if err != nil {
	...
	err = a.DB.QueryRow(riskQuery, params...).Scan(&atRisk)
	if err != nil {
	...
	rows, err := a.DB.Query(tableQuery, params...)
	if err != nil {
	...
	var devices []models.AtRiskDevice
	for rows.Next() {
		var d models.AtRiskDevice
	...
	response := models.AtRiskResponse{
		KPI: models.AtRiskKPI{
			TotalDevices:  total,
			AtRisk:        riskCount,
