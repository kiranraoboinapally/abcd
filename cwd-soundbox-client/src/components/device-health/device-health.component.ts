...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, HighchartsChartModule], // Removed HttpClientModule
  template: `
...
export class DeviceHealthComponent implements OnInit, OnChanges {
  ...
  fetchData() {
    ...
    this.apiService.getDeviceHealthData(params).subscribe({
      next: (res) => {
        this.data.set(res || []);
      },
  ...
  fetchAtRiskData() {
    ...
    this.apiService.getAtRiskKPIs(params).subscribe({
      next: (res) => {
        const atRiskDevices = res || [];
