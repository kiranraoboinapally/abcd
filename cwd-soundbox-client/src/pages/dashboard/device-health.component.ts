...
@Component({
  ...
  template: `
    <div class="flex flex-nowrap gap-2 p-1 bg-gray-100 min-h-[648px]">
      <div
        class="w-[400px] h-[648px] bg-white rounded-lg shadow-md flex flex-col box-border p-2"
      >
        <div
          class="w-full h-[27px] font-bold text-lg text-gray-800 mb-2 select-none"
        >
          At Risk Devices
        </div>
        <table class="w-full border-collapse text-sm select-none">
          <thead>
            <tr class="h-[38px] bg-gray-200">
              <th
                class="p-3 text-center font-bold text-gray-800 border-b border-gray-400"
              >
                Device ID
              </th>
              <th
        ...
        <div
          class="w-full flex-grow overflow-y-auto mt-1 border border-gray-300 rounded"
        >
          <table class="w-full border-collapse text-sm">
            <tbody>
              <tr
                *ngFor="let item of paginatedAtRiskData()"
              ...
              >
                ...
                <td class="px-4 py-3 text-center whitespace-nowrap">
                  {{ item.device_bs | number : "1.1-1" }}%
                </td>
              </tr>
              <tr *ngIf="atRiskData().length === 0">
                <td colspan="2" class="text-center text-gray-500 p-4">
                  No at-risk devices found.
                </td>
              </tr>
            </tbody>
          </table>
        ...
        <div
          class="flex justify-end items-center gap-2 text-xs mt-1 select-none border-t border-gray-300 pt-3 mt-auto"
        >
          <button
            (click)="atRiskPrevPage()"
            [disabled]="atRiskCurrentPage() === 1"
            class="px-2 py-1 text-lg leading-none rounded hover:bg-blue-100 disabled:text-gray-300"
          >
            ‹
          </button>
          <span
            >{{ atRiskStartItem() }} – {{ atRiskEndItem() }} of
            {{ atRiskTotalItems() }}</span
          >
          <button
      ...
      <div class="flex flex-col gap-2 flex-grow h-[648px] box-border">
        <div
          class="h-[300px] p-2 bg-white rounded-lg shadow-md flex flex-col items-start"
        >
          <div
            class="w-full h-12 flex items-center border-b border-gray-300 mb-3"
          >
            <h2 class="m-0 font-bold text-xl text-gray-700">
              Battery Status Over Time{{
                selectedDevice() ? " for Device " + selectedDevice() : ""
              }}
            </h2>
          </div>
          <highcharts-chart
            *ngIf="dataLoaded() && filteredData().length > 0; else noDataChart"
        ...
        <div
          class="flex-1 bg-white rounded-lg shadow-md flex flex-col p-2 box-border"
        >
          <div
            class="w-full h-[27px] font-bold text-lg text-gray-800 mb-2 select-none"
          >
          ...
          <table class="w-full border-collapse text-sm select-none">
            <thead>
              <tr class="h-[38px] bg-gray-200">
                <th class="p-3 text-center font-bold text-gray-800 border-b">
                  Block
                </th>
                <th class="p-3 text-center font-bold text-gray-800 border-b">
                  Device ID
          ...
          <div
            class="w-full flex-grow mt-1 border border-gray-300 rounded overflow-y-auto"
          >
            <table class="w-full border-collapse text-sm">
              <tbody>
                <tr
                  *ngFor="let item of paginatedData()"
                  class="h-[48px] border-b border-gray-300 hover:bg-gray-100"
                >
                  ...
                  <td class="px-4 py-3 text-center">
                    {{ item.start_battery_level }}%
                  </td>
                  <td class="px-4 py-3 text-center">
                    {{ item.end_battery_level }}%
                  </td>
...
export class DeviceHealthComponent implements OnInit, OnChanges {
  ...
  dataLoaded = signal<boolean>(false); // New signal to track data loading
  ...
  ngOnChanges(changes: SimpleChanges): void {
    const shouldRefetch =
      changes["deviceFilter"] ||
      changes["chargingStatusFilter"] ||
  ...
  fetchData() {
    this.dataLoaded.set(false); // Reset loading state
    let url = `${this.apiUrl}/getDeviceHealthData`;
    const params: { [key: string]: string } = {};
    if (this.searchTerm) params["search"] = this.searchTerm;
    if (this.deviceFilter) params["device_id"] = this.deviceFilter;
    if (this.chargingStatusFilter)
      params["charging_status"] = this.chargingStatusFilter;
    if (this.statusFilter)
      params["is_anomaly"] = this.statusFilter === "Anomaly" ? "Yes" : "No";
    ...
    this.http.get<{ battery_health: DeviceHealth[] }>(url).subscribe({
      next: (res) => {
        this.data.set(res.battery_health || []);
        this.dataLoaded.set(true); // Set loading state to true
      },
      error: (err) => {
        console.error("Error fetching device health data:", err);
        this.dataLoaded.set(true); // Set loading state even on error
      },
    });
  }
  ...
  atRiskTotalPages = computed(
    () => Math.ceil(this.atRiskTotalItems() / this.atRiskPerPage) || 1
  );
  atRiskNextPage = () =>
    this.atRiskCurrentPage.update((p) =>
      Math.min(p + 1, this.atRiskTotalPages())
    );
  atRiskPrevPage = () =>
    this.atRiskCurrentPage.update((p) => Math.max(p - 1, 1));
  atRiskStartItem = computed(() =>
    this.atRiskTotalItems() === 0
      ? 0
      : (this.atRiskCurrentPage() - 1) * this.atRiskPerPage + 1
  );
  atRiskEndItem = computed(() =>
    Math.min(this.atRiskCurrentPage() * this.perPage, this.atRiskTotalItems())
  );
  ...
  chartOptions = computed(
    (): Highcharts.Options => ({
      ...
      tooltip: {
        headerFormat:
          '<span style="font-size: 12px">Time: {point.key:%e %b %Y, %H:%M}</span><br/>',
        pointFormat:
          "<b>Device ID:</b> {series.options.custom.deviceId}<br/>" +
